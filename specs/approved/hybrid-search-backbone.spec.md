...
- Users can refine a broad query with lightweight hints such as `img`, `pdf`, `in:library`, or `today`.
...
- In scope: second-level intents for type, scope, and time
...
- As a user, I can add `in:downloads`, `in:library`, `today`, or `img` to tighten a search without learning a complex syntax.
...
- [ ] Intent parsing supports type refiners, `in:<scope>` refiners, and time refiners such as `today`, `yesterday`, and `recent`.
